{\rtf1\ansi\ansicpg936\cocoartf1343\cocoasubrtf160
{\fonttbl\f0\fswiss\fcharset0 Helvetica;}
{\colortbl;\red255\green255\blue255;}
\margl1440\margr1440\vieww10800\viewh8400\viewkind0
\pard\tx566\tx1133\tx1700\tx2267\tx2834\tx3401\tx3968\tx4535\tx5102\tx5669\tx6236\tx6803\pardirnatural

\f0\fs24 \cf0 Operate the program as following:\
\
1. Open the terminal\
2. go to the directory containing the program (by $ cd \'85)\
3. type command \'93make clean\'94\
4. type command \'93make build\'94\
5. type command \'93make check MECHANISM=m X=x N=n\'94 \
    (where m is one mechanism of sequential/select/poll/epoll\
                x is power x of e^x you want to calculate\
                n is the number of terms you want to expend to estimate the corresponding e^x)\
6. get the result\
\
* For select mechanism, the limitation would be 1024 for n}